...
case class LineWall(p: Point2D, q: Point2D) extends Wall:
    ...
    def transform(xform: Transform): Wall = LineWall(xform.of(p), xform.of(q))

    private def intersect(p0: Point2D, p1: Point2D, q0: Point2D, q1: Point2D): Double = {
        val r = p1 - p0
        val s = q1 - q0
...
case class PointWall(p: Point2D) extends Wall:
    ...
    def transform(xform: Transform): Wall = PointWall(xform.of(p))

    private def intersectCircle(p0: Point2D, p1: Point2D, q: Point2D, r: Double): Double = {
        val d = p1 - p0
        val v = p0 - q
